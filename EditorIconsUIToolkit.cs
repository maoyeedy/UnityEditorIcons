...
namespace Project
{
    ...
    {
        private static string[] IcoList => EditorIconsList.IcoList;
        ...
        private void DiscoverIcons()
        {
            ...
            var knownNames = new HashSet<string>(IcoList);

            var discovered = Resources.FindObjectsOfTypeAll<Texture2D>()
                .Select(t => t.name)
                .Where(name => !knownNames.Contains(name));

            var combinedNames = IcoList.Concat(discovered).Distinct();
        ...
        private void BuildToolbar(VisualElement root)
        {
            ...
            {
                style = { width = 180, marginLeft = 5, alignSelf = Align.Center }
            };
        ...
        private void UpdateCategoryDropdownChoices()
        {
            ...
            var filteredIcons = _iconCache.Where(x => x.IsBig == _viewBigIcons).ToList();

            var allDisplay = $"All ({filteredIcons.Count})";
            _categories.Add(allDisplay);
            _categoryMapping[allDisplay] = "All";

            var grouped = filteredIcons.GroupBy(x => x.Category).ToList();
            var uniqueCategories = grouped.Where(c => c.Key != "General").OrderBy(c => c.Key);

            foreach (var group in uniqueCategories)
            {
                var display = $"{group.Key} ({group.Count()})";
                _categories.Add(display);
                _categoryMapping[display] = group.Key;
            }

            var generalGroup = grouped.FirstOrDefault(g => g.Key == "General");
            if (generalGroup != null)
            {
                var display = $"General ({generalGroup.Count()})";
                _categories.Add(display);
                _categoryMapping[display] = "General";
            }
